...
extern uint8_t diagonal_toggled;
...
void referee_processing_task(void *argument) {
	...
	uint16_t last_hp = ref_robot_data.max_HP;
...
//						}
		...
		if (diagonal_mode) {
			...
			// if the diagonal mode has just been toggled on
			if (diagonal_toggled) {
				start_time = curr_time;
				toggle_time = start_time + DIAGONAL_TOGGLE_TIME;
				damage_taken = 1;
			}
			// if the robot takes projectile damage and some time has passed since last hit
			if ((last_hp - ref_robot_data.remain_HP >= 10) && (toggle_time < curr_time)) {
				start_time = curr_time;
				toggle_time = start_time + DIAGONAL_TOGGLE_TIME;
				damage_taken = 1 - damage_taken;
			}

			if (start_time + DIAGONAL_TIME < curr_time) {
				damage_taken = 0;
			}
		} else {
			damage_taken = 0;
		...
		if (last_hp != ref_robot_data.remain_HP) {
			last_hp = ref_robot_data.remain_HP;
		}
