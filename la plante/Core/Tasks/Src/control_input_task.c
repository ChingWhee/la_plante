...
uint8_t song_queued = 0;
// make sure that the last_trig time is updated
uint8_t last_trig_checked = 0;
		...
		if (rc_check) {
			...
			if (remote_cmd.right_switch == ge_RSW_SHUTDOWN) {
				...
				case REMOTE_CTRL_MODE:
					remote_spin(remote_cmd.side_dial);
...
float chassis_center_yaw(double target) {
	speed_pid(target, can_motors[YAW_MOTOR_ID - 1].angle_data.adj_ang,
			&yaw_pid_data);
...
void remote_spin(int16_t left_dial_input) {
...
//		else if (left_dial_input > 330) {
//			if (HAL_GetTick() - last_trig_time > 2000 && !song_queued && last_trig_checked) {
//				temp_msg = song;
//				song_queued = 1;
//				xQueueSendToBack(buzzing_task_msg, &temp_msg, 0);
//			}
...
//		else if (left_dial_input < -330) {
//			if (HAL_GetTick() - last_trig_time > 3000 && !song_queued && last_trig_checked) {
//				temp_msg = song;
//				song_queued = 1;
//				xQueueSendToBack(buzzing_task_msg, &temp_msg, 0);
//			}
